...
description: >
  ...
  user wants to visualize workflows, architectures, or concepts as Excalidraw
  diagrams. Also use when generating diagrams for Jupyter notebooks that need
  SVG embedding — the skill supports both PNG (via render script) and SVG export
  (via _excalidraw_to_svg.js for notebook use). Trigger on: "draw a diagram",
  "create an excalidraw", "visualize this architecture", "make a flow diagram",
  "画图", "绘制架构图", or when reimpl-tutorial skill selects SVG as the diagram method.
---
...
Find the `references/` directory inside this skill's folder (sibling to this
SKILL.md), then run:

```bash
cd <this-skill-dir>/references && uv run python render_excalidraw.py <path-to-file.excalidraw>
```
...
### Alternative: SVG Export (for Jupyter Notebooks)

When the diagram will be embedded in a Jupyter notebook (via `![](path.svg)`), export to SVG instead of PNG for better quality and scalability.

A reference converter script is bundled at `references/_excalidraw_to_svg.js`.
Copy it into the project's diagrams directory and run:

```bash
# Convert one file
node _excalidraw_to_svg.js diagram.excalidraw  # → diagram.svg

# Convert all .excalidraw files in the directory
node _excalidraw_to_svg.js --all
```

The converter handles: `rectangle`, `diamond`, `ellipse`, `text`, `arrow`, `line` elements. It computes a tight viewBox with 20px padding and renders shapes before text (so text always appears on top).

Then in the notebook markdown cell:
```markdown
![Diagram title](../diagrams/diagram-name.svg)

> *图注：Add a descriptive caption explaining what the diagram shows.*
```

**SVG vs PNG tradeoff:**
- SVG: crisp at any size, smaller file, no setup needed — best for notebooks and documentation
- PNG: required for presentations or platforms that don't render SVG inline
